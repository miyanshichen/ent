  ...
  <div class="login">
    <div class="form-box">
      ...
      <el-form :model="regForm" ref="regForm" :rules="regRules">
        <!-- <el-form-item prop="name"><el-input v-model="regForm.name" title="用户名"></el-input></el-form-item> -->
        <el-form-item prop="name"><mc-input :val="regForm.name" title="用户名" @input-change="(val) => {regForm.name = val}"></mc-input></el-form-item>
        <el-form-item prop="mobile"><mc-input :val="regForm.mobile" title="手机号" @input-change="(val) => {regForm.mobile = val}"></mc-input></el-form-item>
        <el-form-item prop="rid"><mc-input :val="regForm.rid" title="推荐人" @input-change="(val) => {regForm.rid = val}"></mc-input></el-form-item>
        <el-form-item prop="password"
          ><mc-input :val="regForm.password" title="密码" type="password" @input-change="(val) => {regForm.password = val}"></mc-input
        ></el-form-item>
        <el-form-item prop="password2"
          ><mc-input :val="regForm.password2" title="确认密码" type="password" @input-change="(val) => {regForm.password2 = val}"></mc-input
        ></el-form-item>
        <el-form-item prop="vcode"
          ><mc-input :val="regForm.vcode" :mobile="regForm.mobile" :reg="true" title="短信验证码" :code="true" @input-change="(val) => {regForm.vcode = val}"></mc-input
        ></el-form-item>
  ...
  methods: {
    register() {
      this.$refs.regForm.validate(async valid => {
        if (valid) {
          this.$axios.post(this.$https.api + '/signUp', this.regForm).then(res => {
            if (res.status === 1) {
              this.$message({
                message: '注册成功',
                type: 'success'
