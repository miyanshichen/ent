    ...
    <div class="container white-bg mc-mtb-20 mc-overflow">
      <div class="withdraw">
        <div class="mc-flex mc-flex-direction-column mc-items-center">
          <el-form class="mc-w-100" :model="buyForm" :rules="buyRules" ref="buyForm">
            <!-- <el-form-item>
            ...
            <el-form-item prop="password">
              <mc-input
                ...
                @input-change="(val) => {buyForm.password = val}"
                type="password"
              ></mc-input>
            </el-form-item>
            <!-- <el-form-item>
          ...
          <div class="mc-w-100 main-text mc-size12">可用USDT：{{canUseUSDT}}</div>
          <div class="total mc-flex mc-flex-justify-between mc-w-100 mc-mb-15 main-text">
            <span>交易额</span>
            <span>{{useUSDT}}USDT</span>
          </div>
          <el-button type="primary" class="mc-w-100" @click="transSub()">提交</el-button>
        </div>
      </div>
    </div>
        ...
        <el-col :span="24">
          <div class="grid-content bg-purple hidden-xs-only">
            <div class="logs-title mc-flex mc-flex-justify-between mc-prl-15">
              <span>购币记录</span>
              <router-link to="/finance/trans/logs" class="gray3-text mc-size12">查看更多></router-link>
            </div>
...
export default {
  ...
  methods: {
    async getRadio() {
      await this.$axios
        .get(this.$https.api + "/user/exchangeInfo?from_coin_id=3&coin_id=1")
        .then(res => {
          if (res.status === 1) {
    ...
    async getUSDT () {
      await this.$axios.get(this.$https.api + '/coin/lists?type=2&name=USDT').then(res => {
        if (res.status === 1) {
