...
export default {
  ...
  methods: {
    async coinInfo() {
      await this.$axios.get(this.$https.api + '/coin/lists?type=1&name=USDT').then(res => {
        console.log(res)
      })
    },
    async getLogs() {
      await this.$axios.get(this.$https.api + '/coin/record?type=1&page=1&page_size=10&coin_id=1').then(res => {
        console.log(res)
      })
    },
    bindQRCode() {
